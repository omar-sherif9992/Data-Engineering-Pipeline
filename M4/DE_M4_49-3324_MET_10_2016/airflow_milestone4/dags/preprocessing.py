...
def save_lookup_table(file_name: str):
    ...
    save_dataframe(concatenated_df, file_name, 'csv')
...
def empty_folders():
    ...
    """
    for folder_path in [FIGURES_DIR, CHECKPOINT_DIR]:
...
def process_location_columns(df: pd.DataFrame):
    """
...
def geocode_and_save_combined_locations(df, output_csv):
    """
    ...
    """
    LOCATION_COLUMNS = ['pu_location', 'do_location']
    location_col_names= LOCATION_COLUMNS
...
def integrate_gps_coordinates(df: pd.DataFrame,location_filename):
    """
    ...
    location_data = pd.read_csv(location_filename)
...
def same_or_diff_neighberhood(df):
    LOCATION_COLUMNS = ['pu_location', 'do_location']
...
def fix_improvement_surcharge(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for key in LOOKUP_TABLE:
        ...
        if 'imputed' in curr_df.columns and 'encoded' in curr_df.columns:
            if len(table) > 0:
                ...
                curr_df['column_name'] = key  # Updated to 'column name'
...
def clean_missing(df):
    ...
    identify_columns_needing_imputation(df, ['lpep_pickup_datetime', 'lpep_dropoff_datetime', ])
    sort_values(df)
    # Call the function to calculate total time
    calculate_total_time(df, 'lpep_pickup_datetime', 'lpep_dropoff_datetime')

    process_location_columns(df)
...
def encoding(df,lookup_table_path):
    ...
    encode_location(df)
    save_lookup_table(lookup_table_path)

    return df



def extract_gps(df,locatiion_filename):

    geocode_and_save_combined_locations(df,locatiion_filename)
    df = integrate_gps_coordinates(df,locatiion_filename)
